...
const columns: Column<ContactContent>[] = [
  { header: 'Title', accessor: 'title', className: 'font-semibold text-navy max-w-[280px] truncate' },
  { header: 'Send Message', accessor: 'sendMessage', className: 'max-w-[200px] truncate' },
  { header: 'Contact Info', accessor: 'contactInfo', className: 'max-w-[200px] truncate' },
  {
    header: 'Published',
    accessor: (row) => (
    ...
    <div className="flex flex-col gap-5 flex-1 min-h-0">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <p className="text-text-secondary text-sm font-[family-name:var(--font-poppins)]">
          {data
            ? 'Edit the contact page content that appears on the website.'
            : 'No contact content yet. Create it to get started.'}
        </p>
      ...
      <ContactModal isOpen={modalOpen} onClose={() => setModalOpen(false)} data={data ?? null} />
    </div>
  );
}
